...
function createChainTask(chain) {
  ...
  gulp.task('clone-ds-' + chain, function(cb) {
    if (!fs.existsSync('./git_components/ds-' + chain)) {
      ...
      exec(arg, {
        cwd: process.cwd() + '/git_components/ds-' + chain
      },
        function(error, stdout, stderr) {
...
gulp.task('bower', function() {
  var bowerFile = './bower-' + config.branch + '.json';
  fs.createReadStream(bowerFile).pipe(fs.createWriteStream('./bower.json'));
  return bower({
    cmd: 'update'
  });
})
