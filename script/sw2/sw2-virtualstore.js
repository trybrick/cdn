		...
		init : function (options) {
			...
			return this.each(function () {
				...
				$modal.css({
					...
					'left' : (window.devicePixelRatio >= 2) ? 0 : 50 + '%',
...
(function ($, window, undefined) {
  ...
  didOpen = false,
	...
	init = function (id, selector, options) {
		...
		if (//location.hostname.indexOf('beta') > 0 || //if beta, show the lightbox
			getCookie("shopperwelcome2") == null) { //if the cookie is set, don't show the lightbox
...
$('.gsnunit').each(function (index, element) {
  ...
  info = eval('(' + element.getAttribute('data-info') + ')');
  var targeting = eval('(' + element.getAttribute('data-targeting') + ')');

  var zone = info.unitname.split('/')[1];
  var uname = info.network + '/' + info.unitname.split('/')[0];
  var unitNameRoot = '';
  var departments = targeting.dept;
  var brand = '';
  var page = targeting.kw;

  //setMainAttr(zone, uname, unitNameRoot, departments, brand, page);
});
...
if(chainId){
  ...
  jQuery.gsnSw2({
    ...
    onClose: function (didDisplay) {
      ...
      if (didDisplay) {
        //if (!Gsn.Advertising.processCampaign()) {
        //    Gsn.Advertising.refreshAdPodsWithDept();
        //}
        //Gsn.Advertising.clickBrand(Gsn.Advertising.getBrand());
        $.gsnDfp({
          dfpID: id,
