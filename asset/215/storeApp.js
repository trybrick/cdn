var storeApp = angular
    ...
    .config(['$routeProvider', '$locationProvider', '$sceDelegateProvider', '$sceProvider', '$httpProvider', 'FacebookProvider', '$analyticsProvider', function ($routeProvider, $locationProvider, $sceDelegateProvider, $sceProvider, $httpProvider, FacebookProvider, $analyticsProvider) {
      ...
      gsn.config.DisableLimitedTimeCoupons = true;
      ...
      $routeProvider
          ...
          .when('/profile/rewardcardupdate', {
            templateUrl: gsn.getThemeUrl('/views/profile-edit-roundy.html'),
            controller: 'RoundyProfileCtrl',
            requireLogin: true,
            caseInsensitiveMatch: true
          })
...
storeApp.controller('ContactUsCtrl', ['$scope', 'gsnProfile', 'gsnApi', '$timeout', 'gsnStore', '$interpolate', '$http', function ($scope, gsnProfile, gsnApi, $timeout, gsnStore, $interpolate, $http) {
  ...
  function getData() {
    return [
        {
          "Value": "Fresh Perks Card",
          "Text": "Fresh Perks Card",
          "ParentOption": ""
        },
        {
...
storeApp.controller('RoundyProfileCtrl', ['$scope', 'gsnProfile', 'gsnApi', '$timeout', 'gsnStore', '$rootScope', '$q', '$http', function ($scope, gsnProfile, gsnApi, $timeout, gsnStore, $rootScope, $q, $http) {
  $scope.activate = activate;
  $scope.profile = {};
  ...
  $scope.isSubmitting = false;    // true if we're waiting for result from server
  $scope.showRewardUpdate = false;
  $scope.lastNameDoesNotMatch = false;
  $scope.isLoading = true;
  $scope.updateSuccess = false;

  function activate() {
    // if invalid ExternalId, return
    ...
    gsnProfile.getProfile().then(function (p) {
      if (p.success) {
        ...
        if (gsnApi.isNull(pf.ExternalId, '').length > 2) {
          gsnStore.getRoundyRewardProfile(pf.ExternalId).then(function (rp) {
            $scope.isLoading = false;
            if (rp.success) {
              $scope.profile = angular.copy(rp.response);

              $scope.lastNameDoesNotMatch = (pf.LastName.toLowerCase() != rp.response.LastName.toLowerCase());
              if ($scope.lastNameDoesNotMatch) {
                $scope.showRewardUpdate = false;
              }
            }
          });
  ...
  $scope.updateProfile = function () {
    ...
    if ($scope.myForm.$valid) {
      ...
      // update profile to new api
      updateProfile(profile)
          .then(function (result) {
            $scope.isSubmitting = false;
            $scope.isValidSubmit = result.success;
            $scope.showRewardUpdate = !$scope.isValidSubmit;
            $scope.updateSuccess = true;
          });
    }
  };
  ...
  function updateProfile(profile) {
    ...
    gsnApi.getAccessToken().then(function () {
      var url = gsnApi.getProfileApiUrl().replace('/profile', '/roundy/ProfileUpdate/') + gsnApi.getChainId();

      $http.post(url, payload, { headers: gsnApi.getApiHeaders() }).success(function (response) {
        gsnStore.clearRewardProfile();
